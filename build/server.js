...
server.registerTool("ctx_execute_file", {
    ...
    description: "Read a file (or multiple files / directory) and process it without loading contents into context. Only your printed summary enters context.\n\n" +
        "**Modes:**\n" +
    ...
    inputSchema: z.object({
        ...
        code: z
            .string()
            .describe("Code to run in sandbox. Single-file: use FILE_CONTENT / file_path. Multi-file: use `files` map (shell: FILE_0_PATH/FILE_0_CONTENT/FILE_COUNT). Print summary via console.log/print/echo/IO.puts."),
        timeout: z
            .number()
            .optional()
            .default(30000)
    ...
    if (modeCount === 0) {
        return trackResponse("ctx_execute_file", {
            content: [{ type: "text", text: "One of path, paths, or dir is required." }],
            isError: true,
        });
    }
    ...
    try {
        ...
        if (dir && result.resolvedPaths && result.resolvedPaths.length > 0) {
            ...
            for (const absPath of result.resolvedPaths) {
                try {
                    store.index({ path: absPath, source: absPath });
                }
                catch { /* skip unindexable */ }
            }
        }
        ...
        if (result.exitCode !== 0) {
            ...
            if (intent && intent.trim().length > 0 && Buffer.byteLength(output) > INTENT_SEARCH_THRESHOLD) {
                ...
                return trackResponse("ctx_execute_file", {
                    content: [
                        { type: "text", text: intentSearch(output, intent, isError ? `${inputLabel}:error` : inputLabel) },
                    ],
                    isError,
                });
            }
        ...
        if (intent && intent.trim().length > 0 && Buffer.byteLength(stdout) > INTENT_SEARCH_THRESHOLD) {
            ...
            return trackResponse("ctx_execute_file", {
                content: [
                    { type: "text", text: intentSearch(stdout, intent, inputLabel) },
                ],
            });
        }
        return trackResponse("ctx_execute_file", {
    ...
    inputSchema: z.object({
        commands: z
            ...
            if (Array.isArray(arr)) {
                return arr.map((item, i) => typeof item === "string"
