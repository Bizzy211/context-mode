...
export class PolyglotExecutor {
    ...
    #resolveFiles(opts) {
        ...
        if (opts.paths) {
            ...
            for (const p of opts.paths) {
                ...
                if (!isAbsolute(p)) {
                    const rootWithSep = this.#projectRoot.endsWith(sep) ? this.#projectRoot : this.#projectRoot + sep;
                    if (!abs.startsWith(rootWithSep) && abs !== this.#projectRoot)
                        continue;
                }
                let buf;
                try {
                ...
                if (bytes > MAX_FILE_BYTES || totalBytes + bytes > MAX_INJECT_BYTES)
                    continue;
                totalBytes += bytes;
            ...
            if (relPaths.length > MAX_FILE_COUNT) {
                throw new Error(`Directory contains more than ${MAX_FILE_COUNT} files, which exceeds the limit of ${MAX_FILE_COUNT}. ` +
            ...
            for (const rel of relPaths.sort()) {
                ...
                const content = buf.toString("utf-8");
                const bytes = Buffer.byteLength(content);
                if (totalBytes + bytes > MAX_INJECT_BYTES)
                    continue; // skip injection, but still indexed
                totalBytes += bytes;
     ...
     */
    #safeReaddirFiles(dir, limit, base = "") {
        const files = [];
        let items;
        try {
     ...
     */
    #wrapWithMultipleFiles(fileMap, language, code) {
        ...
        switch (language) {
            ...
            case "rust": {
                const inserts = entries.map(([k, v]) => `    files.insert(${JSON.stringify(k)}.to_string(), ${JSON.stringify(v)}.to_string());`).join("\n");
